...
public final class Animator extends JPanel implements ActionListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // draw line in between balls
        g2.setColor(Color.BLUE);
        Shape line = new Line2D.Double(
                model.balls[0].position.x * pixelsPerMeter,
                this.getHeight() - (model.balls[0].position.y * pixelsPerMeter),
                model.balls[1].position.x * pixelsPerMeter,
                this.getHeight() - (model.balls[1].position.y * pixelsPerMeter));
        g2.draw(line);
